...
2. 자료구조
 - ArrayList<Integer> 2개: 각 크레인의 최대 무게, 각 박스의 무게
...
public class Main {
	static int n;				// 크레인 개수
	static List<Integer> craneList = new ArrayList<>();		// 각 크레인의 최대 무게 제한
	static int m;				// 박스 개수
	static List<Integer> boxList = new ArrayList<>();		// 각 박스의 무게
	...
	static void solution() {
		...
		while (!boxList.isEmpty()) {
			...
			for (int i = 0; i < n; ) {				// 주의) for 문에 i++ 없음
				if (boxIdx == boxList.size())		// 마지막 박스까지 확인한 경우
					break;

				if (craneList.get(i) >= boxList.get(boxIdx)) {
					boxList.remove(boxIdx);
					i++;
				}
				else			// 해당 크레인으로 해당 박스 못 옮기는 경우
					boxIdx++;	// => 다음 박스 옮길 수 있는지 확인
			}

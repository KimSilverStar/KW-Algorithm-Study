...
1. 아이디어
 1) DP 배열 정의: String[] dp
   - dp[cost]: cost 원 금액 내로 만들 수 있는 최대 숫자 문자열
   - 출력, 최대 숫자: dp[m]

 2) 규칙 및 점화식
   - 초기식: 입력 costs[]의 각 원소 cost 에 대해, dp[cost] = String(i)
   - 1원 ~ m원 까지 각 금액으로 만들 수 있는 최대 숫자 문자열로 dp[] 채우기
...
public class Main {
	static int n;					// 0 ~ (n-1) 숫자
	static int[] costs;				// 각 숫자의 구매 금액
	static int m;					// 보유 금액
	static String maxNumStr;		// 출력, 최대 숫자
	static String[] dp;
	...
	static void solution() {
		...
		for (int totalCost = 1; totalCost <= m; totalCost++) {
			for (int i = 0; i < n; i++) {
				...
				dp[totalCost] = getMaxStr(dp[totalCost], dp[totalCost - cost] + dp[cost]);
			}
		}
